...
if (-not (Test-Path "conf")) {
    Write-Host "❌ conf/ directory not found" -ForegroundColor Red
    exit 1
}
...
if (Test-Path "conf/settings.json") {
    Write-Host "⬆️  Uploading settings.json..." -ForegroundColor Yellow
    ...
    Write-Host "✅ settings.json uploaded" -ForegroundColor Green
} else {
    Write-Host "⚠️  settings.json not found, skipping" -ForegroundColor Yellow
}

# Upload channels.json
if (Test-Path "conf/channels.json") {
    Write-Host "⬆️  Uploading channels.json..." -ForegroundColor Yellow
    ...
    Write-Host "✅ channels.json uploaded" -ForegroundColor Green
} else {
    Write-Host "⚠️  channels.json not found, skipping" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "✅ Configuration uploaded and application restarted!" -ForegroundColor Green
Write-Host ""
